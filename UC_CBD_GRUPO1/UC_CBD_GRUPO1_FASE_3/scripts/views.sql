...
CREATE OR REPLACE VIEW V_InfoCliente AS
    SELECT /*+ PARALLEL(DEFAULT)*/ c.vc_nome AS "Nome", c.nb_nif AS "NIF", c.vc_email AS "Email", c.vc_profissao AS "Profissão", c.dt_DataNascimento AS "Data de Nascimento", c.nb_idade AS "Idade",COUNT(e.nb_nendereco) AS "Nº de Moradas"
...
CREATE OR REPLACE VIEW V_CarteiraSaldos AS
    SELECT /*+ PARALLEL(DEFAULT)*/ c.nb_ncliente AS "NumCliente", c.vc_nome AS "NomeCliente", ct.nb_iban AS "IBAN", ct.nb_saldo || ' €' AS "Saldo", p.vc_tipoproduto AS "Produto"
...
CREATE OR REPLACE VIEW V_LavagemDinheiroValor AS
    SELECT /*+ PARALLEL(DEFAULT)*/ nb_numerotransacao AS "NumTransação", nb_iban AS "IBAN", nb_ncliente AS "NumCliente", nb_valor AS "Valor", nb_ibanrecetor AS "IBAN Recetor", dta_dtatransacao AS "Data", nb_operacao AS "Operação"
    FROM transacao 
...
CREATE OR REPLACE VIEW V_LavagemDinheiroClienteEXT AS
    SELECT /*+ PARALLEL(DEFAULT)*/ t.nb_numerotransacao AS "NumTransação", t.nb_iban AS "IBAN", t.nb_ncliente AS "NumCliente", t.nb_valor AS "Valor", t.nb_ibanrecetor AS "IBAN Recetor", t.dta_dtatransacao AS "Data", e.vc_pais AS "País"
...
CREATE OR REPLACE VIEW V_ClientesPorAgencia AS 
    SELECT /*+ PARALLEL(DEFAULT)*/  a.nb_nagencia AS "Agência", count(c.nb_ncliente) AS "Clientes"
...
CREATE OR REPLACE VIEW V_TopClientesBanco AS
    SELECT /*+ PARALLEL(DEFAULT)*/  c.nb_ncliente AS "NumCliente", c.vc_nome AS "Nome", SUM(ct.nb_saldo) || ' €' AS "Saldo total"
...
--SELECT * From V_TopClientesBanco;
--DROP VIEW V_topclientesbanco;


--View que mostra os top 10 clientes por agência
CREATE OR REPLACE VIEW V_TopClientesPorAgencia AS
    SELECT /*+ PARALLEL(DEFAULT)*/ a.nb_nagencia AS "Agencia", c.nb_ncliente AS "NumCliente", c.vc_nome AS "Nome", SUM(ct.nb_saldo) AS "Saldo total"
