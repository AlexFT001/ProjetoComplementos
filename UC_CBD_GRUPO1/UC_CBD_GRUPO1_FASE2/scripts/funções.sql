...
CREATE OR REPLACE FUNCTION titularOrdem (numeroConta conta.nb_iban%type)  RETURN VARCHAR2 IS
    ...
    titular varchar2(20);
BEGIN
    SELECT  COUNT(nb_iban)
    INTO pessoasConta
    FROM titular
    WHERE nb_iban = numeroConta;
...
CREATE OR REPLACE FUNCTION numerodecimalaleatorio (min number, max number) RETURN NUMBER IS
    resultado number;
BEGIN
    SELECT TRUNC(DBMS_RANDOM.VALUE(min, max), 2)
    INTO resultado
    FROM DUAL;

    RETURN  resultado;
END numerodecimalaleatorio;
/